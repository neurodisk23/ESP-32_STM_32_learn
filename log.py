...
# ═══════════════════════════════════════════════════════════
# PACKET ROUTER — handles one complete, parsed packet
# ═══════════════════════════════════════════════════════════
def _process_packet(pkt_type, data, raw, csv_mgr, reassembler, stats, ntp):
    ...
    if pkt_type == 'audio':
        ...
        for buf_id, rtc, cal, samples in reassembler.get_complete():
            stats.bufs_complete += 1
            rows = build_audio_rows(buf_id, rtc, cal, samples,
            ...
            if samples:
                ...
                avg_sc  = sum(s.get('sc', 0)
                              for s in samples) / len(samples)
                snores  = sum(1 for s in samples if s.get('sn', False))
                print(f"[{now_str}] AUDIO "
                      f"buf:{buf_id:4d} samples:{len(samples):2d} "
                      f"rms:{avg_rms:5.1f}dB centroid:{avg_sc:6.1f}Hz "
                      f"snore:{snores}")
    ...
    # ENVIRONMENT + HEARTBEAT packet
        ...
        stats.env_rows += 1
        stats.update_health(data)
        if data.get('lux_alert'):  stats.lux_alerts  += 1
        if data.get('temp_alert'): stats.temp_alerts += 1

        # Sanity-check calibration noise floor (~30–60 dB SPL).
        noise = data.get('calib_noise_db', 0)
        if data.get('calib_valid') and (noise < 20 or noise > 90):
            print(f"  ⚠️ Calibration noise {noise:.1f}dB looks unusual "
                  f"(expected ~30–60 dB SPL) — consider re-calibrating")
        ...
        print(f"[{now_str}] ENV lux:{lux_str} {th_str} "
              f"batt:{row['batt_v']:.2f}V heap:{row['free_heap']} "
    ...
    # PRESSURE packet
        ...
        p_str = f"{row['pressure']} hPa" if p_valid else "DEAD"
        print(f"[{now_str}] PRESSURE {p_str}")

    # AMBIENT BATTERY ALERT
    ...
    # HUB STATUS
        ...
        print(f"[{now_str}] HUB_STATUS rtc:{data.get('rtc','')}")

    # PONG
    elif pkt_type == 'pong':
        print(f"[{now_str}] PONG from hub rtc:{data.get('rtc','')}")
...
def main():
    ...
    # Persistent byte buffer — accumulate across reads and split
    # on newlines ourselves. readline() with a timeout can return
    # a partial line mid-packet for long (511-byte audio) packets;
    # buffering guarantees we only parse COMPLETE lines.
    rx_buffer = b''
    ...
    while True:
        try:
            ...
            # If no complete line yet, keep reading
            if b'\n' not in rx_buffer:
                if not chunk:
                    # genuinely idle — do maintenance
            ...
            # ── Split into complete lines; keep remainder ──
            lines = rx_buffer.split(b'\n')
            rx_buffer = lines[-1]          # last piece is incomplete
            complete_lines = lines[:-1]    # all but the last are whole

            for line_bytes in complete_lines:
                raw = line_bytes.decode('utf-8', errors='ignore')
                if not raw.strip():
                    continue
                ...
                _process_packet(pkt_type, data, raw,
                                csv_mgr, reassembler, stats, ntp)

            # periodic maintenance after processing a batch
            stats.print_if_due(csv_mgr)
            ntp.check_and_sync()
            csv_mgr.check_date_rollover()
